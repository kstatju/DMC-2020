...
Parallel_function <- function(tr_dt, ts_dt, params, tune_names){
  ...
  Fit_model   = function(train_dt, test_dt, x_feature, val_dt = NULL, respons = "order", 
    ...
    if(!neural){
      x_train = as.matrix(train_dt %>% select_at(.vars = c(hier_var, x_feature)))
      x_test = as.matrix(test_dt %>% select_at(.vars = c(hier_var, x_feature) ))
      ...
      tr_dt = as.data.frame(y_train) %>% bind_cols(as.data.frame(x_train)) %>% as.data.frame()
      ts_dt = as.data.frame(y_test) %>% bind_cols(as.data.frame(x_test)) %>% as.data.frame()
      
      rm(train_dt, test_dt)
    }else{
      
      
      x_val = as.matrix(val_dt  %>% select_at(.vars = c(hier_var, x_feature)))
      x_val_price = as.matrix(val_dt %>% select_at(.vars = price))
      y_val = as.matrix(val_dt  %>% select_at(.vars = respons) )
      
      vl_dt = x_val %>% bind_cols(y_val) %>% as.data.frame()
    ...
    result <- tryCatch({
      ...
      fit = mhglm_ml(formula_f, data = tr_dt, family = as.character(param[[1]]),
                     control = list(diagcov=param[[2]], parallel = F, trace = FALSE))
      
      yhat_train = round(predict(fit, tr_dt))
      yhat_train = ifelse(yhat_train<0, 0, yhat_train)
      yhat_test  = round(predict(fit, ts_dt))
...
parallel_fit <- function(file_name_tr, file_name_ts){
  ...
  if (Fit_Parallel){
    ...
    clexp = list("hier_var", "formula_f", "Parallel_function", "Model", "logtr", "price_var", 
...
Fit_Parallel  = TRUE                   # if want to fit model parallel 
Model         = "Hlm"          # Model used to train
price_var     = "new_price"            # price variable name
logtr         = TRUE
neural        = FALSE

hier_var12      = c("manufacturer",             "brand",
                  "category1",                "category2",             "category3")

for (kk in hier_var12){
  ...
  formula_f<-  as.formula(paste(respons_var, "~" ,
                                 "1 + ", paste(x_features, collapse = " + "), "+",
                                 "(1+", paste(x_features, collapse = " + "), "| ",kk,")",
                                 collape = "" ))
  
  Exp_Params = list(family = c("gaussian", "poisson", "Gamma", "quasipoisson"),
                    diagcov=c(TRUE, FALSE))         # list of hyper-parameters 
  
  # Read data file and 
  # Define the data file as "Train"
  result = parallel_fit(file_name_tr = "final_tr_std_log_promo2_tsw", file_name_ts = "final_ts_std_log_promo2_tsw")
  
  path = "/work/STAT/kanak/hlm/"
  
  write.csv(x = result, file = paste(path, "hlm_res_promo2_",kk,".csv", sep = ""), row.names = F)
