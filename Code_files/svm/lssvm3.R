...
Parallel_function <- function(tr_dt, ts_dt, params, tune_names){
  ...
  Fit_model   = function(train_dt, test_dt, x_feature, val_dt = NULL, respons = "order", 
    ...
    result <- tryCatch({
      ...
      fit = liquidSVM::lsSVM(x = x_train, y = y_train, scale = T, display = 0,
                           partition_choice = 5,
...
logtr         = TRUE
neural        = FALSE
x_features    = c(  "manufacturer_std",             "brand_std",
...
Exp_Params = list(min_gamma = c(50),
                  max_gamma = c(75),
                  min_lambda = c(1E-4),
                  max_lambda = c(100),
                  KERNEL = c("POISSON"))         # list of hyper-parameters         # list of hyper-parameters 
...
path = "/work/STAT/kanak/svm/"

write.csv(x = result, file = paste(path, "lssvm_res3.csv", sep = ""), row.names = F)
