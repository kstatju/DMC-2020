...
Parallel_function <- function(tr_dt, ts_dt, params, tune_names){
  ...
  Fit_model   = function(train_dt, test_dt, x_feature, val_dt = NULL, respons = "order", 
    ...
    result <- tryCatch({
      ...
      fit = glmnet::glmnet(x = x_train, y = y_train, family = "poisson",
                           alpha = param[1], nlambda = 200, standardize = F)
      cv.fit=cv.glmnet(x = x_train, y = y_train, alpha = param[1])
      
      yhat_train = predict(fit, x_train, s = cv.fit$lambda.min)
      yhat_test  = predict(fit, x_test, s = cv.fit$lambda.min)
...
parallel_fit <- function(){
  ...
  if (logtr){
    tr_data = lapply(1:13, function(i) read.csv(file = paste(path, "final_tr_std_log_tsw", i, ".csv", sep = ""), header = T))
    ts_data = lapply(1:13, function(i) read.csv(file = paste(path, "final_ts_std_log_tsw", i, ".csv", sep = ""), header = T))
    
  }else{
    tr_data = lapply(1:13, function(i) read.csv(file = paste(path, "final_tr_std_tsw", i, ".csv", sep = ""), header = T))
    ts_data = lapply(1:13, function(i) read.csv(file = paste(path, "final_ts_std_tsw", i, ".csv", sep = ""), header = T))
  }
...
Model         = "GLM(Poisson)"          # Model used to train
price_var     = "new_price"            # price variable name
logtr         = TRUE
neural        = FALSE
x_features    = c(  "customerRating",           "new_price",             "recommendedRetailPrice",
...
Exp_Params = list(alpha = seq(0.5,0.6, length.out = 50), 
                  lambda = "None")         # list of hyper-parameters 

# Read data file and 
# Define the data file as "Train"
result = parallel_fit()

write.csv(result, file = paste(path, Model, "preams6.csv", sep = ""), row.names = F)