  ...
  Fit_model   = function(train_dt, test_dt, x_feature, val_dt = NULL, respons = "order", 
    ...
    result <- tryCatch({
      ...
      fit = glmnet::glmnet(x = x_train, y = y_train, family = "poisson",
                           alpha = param[1], lambda = param[2], standardize = F)
      # cv.fit=cv.glmnet(x = x_train, y = y_train, alpha = param[1])
      
      yhat_train = predict(fit, x_train, s = param[2])
      yhat_test  = predict(fit, x_test, s = param[2])
...
Exp_Params = list(alpha = 0, 
                  lambda = seq(.01, 10, length.out = 1))         # list of hyper-parameters 

# Read data file and 
# Define the data file as "Train"
result = parallel_fit()

write.csv(result, file = paste(path, Model, "preams1.csv", sep = ""), row.names = F)