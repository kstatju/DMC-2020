data_prep <- function(path, logtr = TRUE, promo_no = 2){
  ...
  orders = orders %>% mutate(logorder = log(order+1),
                             normorder = 2*((order-min(order, na.rm = T))/(max(order, na.rm = T)-min(order, na.rm = T)))-1,
  ...
  if (std_tr){
      ...
      if(logtr){
        write.csv(x = tr_dt, file = paste(path, "final_tr_std_log_promo", promo_no, "_alldt",  ".csv", sep = ""), row.names = F)
        write.csv(x = ts_dt , file = paste(path, "final_ts_std_log_promo", promo_no, "_alldt",  ".csv", sep = ""), row.names = F)
      }else{
        write.csv(x = tr_dt, file = paste(path, "final_tr_std_promo", promo_no, "_alldt", ".csv", sep = ""), row.names = F)
        write.csv(x = ts_dt , file = paste(path, "final_ts_std_promo", promo_no, "_alldt", ".csv", sep = ""), row.names = F)
      }
      
  }else{
    tr_dt = orders12 %>% filter(tr_ts == 1)
    ts_dt =  orders12 %>% filter(tr_ts >1)
    if(logtr){
      write.csv(x = tr_dt, file = paste(path, "final_tr_log_promo", promo_no, "_alldt",  ".csv", sep = ""), row.names = F)
      write.csv(x = ts_dt , file = paste(path, "final_ts_log_promo", promo_no, "_alldt",  ".csv", sep = ""), row.names = F)
    }else{
      write.csv(x = tr_dt, file = paste(path, "final_tr_promo", promo_no, "_alldt", ".csv", sep = ""), row.names = F)
      write.csv(x = ts_dt , file = paste(path, "final_ts_promo", promo_no, "_alldt", ".csv", sep = ""), row.names = F)
    }
  }
...
df = data_prep(path, logtr = T, promo_no = 2, std_tr = T)


tr = df$train_dt_std_log
ts = df$test_dt_std_log
aa = as.data.frame(colSums(is.na(ts)))
# final_tr_std_promo2_alldt.csv, final_ts_std_promo2_alldt.csv, final_tr_std_log_promo2_alldt.csv, final_ts_std_log_promo2_alldt.csv
dt_log2 = rbind(read.csv(paste(path, "final_tr_std_log_promo2_alldt.csv", sep = ""), header = T),
            read.csv(paste(path, "final_ts_std_log_promo2_alldt.csv", sep = ""), header = T))
dt_log3 = rbind(read.csv(paste(path, "final_tr_std_log_promo3_alldt.csv", sep = ""), header = T), 
            read.csv(paste(path, "final_ts_std_log_promo3_alldt.csv", sep = ""), header = T))

dt2 = rbind(read.csv(paste(path, "final_tr_std_promo2_alldt.csv", sep = ""), header = T),
            read.csv(paste(path, "final_ts_std_promo2_alldt.csv", sep = ""), header = T))
dt3 = rbind(read.csv(paste(path, "final_tr_std_promo3_alldt.csv", sep = ""), header = T),
            read.csv(paste(path, "final_ts_std_promo3_alldt.csv", sep = ""), header = T))


library(ggplot2)

ggplot() + geom_density(data=dt2, aes(x=npromoI, group=as.factor(tr_ts), fill=as.factor(tr_ts)),alpha=0.5, adjust=2)+ xlim(-3,5)
ggplot() + geom_density(data=dt3, aes(x=npromoI, group=as.factor(tr_ts), fill=as.factor(tr_ts)),alpha=0.5, adjust=2)+ xlim(-3,5)
