Parallel_function <- function(tr_dt, ts_dt, params, tune_names){
  ...
  Fit_model   = function(train_dt, test_dt, x_feature, val_dt = NULL, respons = "order", 
    ...
    result <- tryCatch({
      ...
      fit = liquidSVM::qtSVM(x = x_train, y = y_train, scale = T, display = 0,
                           weights = param[[6]],
                           partition_choice = 5,
...
path = "/work/STAT/kanak/qtsvm/"

write.csv(x = result, file = paste(path, "qtsvm_res_rbf1.csv", sep = ""), row.names = F)
