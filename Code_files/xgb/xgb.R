  ...
  Fit_model   = function(train_dt, test_dt, x_feature, val_dt = NULL, respons = "order", 
    ...
    result <- tryCatch({
      ...
      fit = xgboost(data = x_train, label = y_train, nrounds = param[[1]],
                    nthread = -1, params = params12, verbose = 0)
...
Exp_Params = list(nrounds = c(50, 100, 200),
                  eta = c(.01, .1, .2, .3), 
                  gamma = c(2, 10, 20, 100), 
                  max_depth = c(6, 8, 10, 12), 
                  min_child_weight = c(3,5), 
                  subsample = c(.6,.75), 
                  colsample_bytree = c(.6, .75),
                  lambda = c(2, 10, 50, 200))        # list of hyper-parameters 
...
write.csv(x = result, file = paste(path, "xgb_res1.csv", sep = ""), row.names = F)
