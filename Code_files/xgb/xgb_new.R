...
Parallel_function <- function(tr_dt, ts_dt, params, tune_names){
  ...
  Fit_model   = function(train_dt, test_dt, x_feature, val_dt = NULL, respons = "order", 
    ...
    if(!neural){
      x_train = as.matrix(train_dt %>% select_at(.vars = x_feature) %>% mutate(new_pr_npromoIW = new_price*(npromoIW+1)))
      x_test = as.matrix(test_dt %>% select_at(.vars = x_feature) %>% mutate(new_pr_npromoIW = new_price*(npromoIW+1)))
    ...
    result <- tryCatch({
      ...
      params12 <- list(booster = "gbtree", objective = "reg:linear",
                       eta = param[[2]], 
                       gamma = param[[3]], 
                       max_depth = param[[4]], 
                       ...
                       lambda = param[[8]])
      fit = xgboost(data = x_train, label = y_train, nrounds = param[[1]],
                    nthread = -1, params = params12, early_stopping_rounds = round(param[[1]]/2),
                    verbose = 0)
...
parallel_fit <- function(file_name_tr, file_name_ts){
  ...
  tr_data = lapply(10:13, function(i) read.csv(file = paste(path, file_name_tr, i, ".csv", sep = ""), header = T))
  ts_data = lapply(10:13, function(i) read.csv(file = paste(path, file_name_ts, i, ".csv", sep = ""), header = T))
...
Exp_Params = list(nrounds = c(50, 100, 1000),
                  eta = c(.15, .2, .25, .3, 0.001),  # amin .15, .2; chance: .25, saba: .3, kanak: 0.001
                  gamma = c(0, .5, 1, 2, 5, 10), 
                  max_depth = c(3,4,5), 
                  min_child_weight = c(3, 4, 5, 6), 
                  subsample = c(.7, .75,.8, 1), 
                  colsample_bytree = c(.45, .5, .55, .6, .65),
                  lambda = c(150, 200, 250, 500))        # list of hyper-parameters 
...
path = "/work/STAT/kanak/xgb/"

write.csv(x = result, file = paste(path, "xgb_res_promo3_final1.csv", sep = ""), row.names = F)
