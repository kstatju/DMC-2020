...
data_prep <- function(path){
  ...
  orders12 = tr_dt %>% bind_rows(ts_dt) %>% as.data.frame()
  
  write.csv(x = (orders12 %>% mutate(logorder = log(order+1))), file = paste(path, "orders_all_features_std.csv", sep = ""), row.names = F)
  write.csv(x = (orders %>% mutate(logorder = log(order+1))), file = paste(path, "orders_all_features.csv", sep = ""), row.names = F)
  ...
  return(list(train_dt_std = tr_dt, test_dt_std = ts_dt, tr_ts = orders1, tr_ts_all_std = orders12,
              tr_ts_all = orders))
}
...
df = data_prep(path)
train_dt = df$train_dt_std %>% as.data.frame()
test_dt = df$test_dt_std %>% as.data.frame()
rm(df)
