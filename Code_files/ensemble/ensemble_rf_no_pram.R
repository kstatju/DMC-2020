Parallel_function <- function(Train, Test, params, tune_names){
  ...
  ##### Please include package for your model here
    ...
    Fit_model   = function(df_tr, df_ts, df_train, df_test, x_feature, respons = "order", 
      ...
      tr = cbind(x_train, y_train) %>% as.data.frame()
      vl = cbind(x_val, y_val) %>% as.data.frame()
      ts = cbind(x_test, y_test) %>% as.data.frame()
      
      #################### Change ##################
      fit = lm(formula = as.formula(paste(respons," ~ .")), data = tr)
        
      ##############################
      yhat_train = predict(fit, tr)
      yhat_train = round(ifelse(yhat_train<0, 0, yhat_train))
      yhat_val = predict(fit, vl)
      yhat_val = round(ifelse(yhat_val<0, 0, yhat_val))
      yhat_test  = predict(fit, ts)
...
Exp_Params = list(none = NULL)         # list of hyper-parameters 
