...
# C1, C2, C3 -> group_by(category 1, 2, or 3)
# c123 -> category 1, 2 and 3
# B -> group_by(brand)
# M -> group_by(Manufacturer)
# I -> group_by(itemID)
# W -> group_by(week)
# Multiple case means, eg
# MC1 -> group_by(manufacturer, category1)
# rating -> based on rating variable
# npromo -> Number of promotion
# morder -> mean number of order
# mncus -> mean number of customer
# mtmncus -> mean of (number of customer * time difference of order)
# mtmmedf -> mean time difference of order
# comp1_ -> complimentary item
# lag1_ -> lag(1) variable





data_prep <- function(path, logtr = FALSE){
  ...
  orders = left_join(left_join(left_join(left_join(left_join(left_join(left_join(orders, 
    ...
    replace_na(list(morderI = 0, mpriceI = 0, morderB = 0, 
                    mpriceB = 0, morderM= 0, mpriceM = 0))
  ...
  orders = left_join(left_join(left_join(left_join(left_join(orders, 
    ...
    replace_na(list(morderBM = 0,    mpriceBM = 0,   morderBC1 = 0,   mpriceBC1 = 0,
                    morderBC2 = 0,   mpriceBC2 = 0,  morderBC3 = 0,   mpriceBC3 = 0,
                    morderBC123 = 0, mpriceBC123 = 0 ))
  ...
  orders = left_join(left_join(left_join(left_join(left_join(orders, 
    ...
    replace_na(list(morderMC1 = 0,    mpriceMC1 = 0,    morderMC2 = 0,     mpriceMC2 = 0,
                    morderMC3 = 0,    mpriceMC3 = 0,    morderMC123 = 0,   mpriceMC123 = 0,
                    morderBMC123 = 0, mpriceBMC123 = 0))
  ...
  orders = left_join(left_join(left_join(left_join(left_join(left_join(left_join(left_join(orders, 
    ...
    replace_na(list(mtmmedfW = 0,   mncusW = 0,     mtmncusW = 0,
                    mtmmedfI = 0,   mncusI = 0,     mtmncusI = 0, 
                    mtmmedfB = 0,   mncusB = 0,     mtmncusB = 0,
                    mtmmedfM = 0,   mncusM = 0,     mtmncusM = 0))
  ...
  orders = left_join(left_join(left_join(left_join(left_join(orders, 
    ...
    replace_na(list(mtmmedfIW =  0,   mncusIW = 0,    mtmncusIW = 0,
                    mtmmedfBW = 0,    mncusBW = 0,    mtmncusBW = 0,
                    mtmmedfBMW = 0,   mncusBMW = 0,   mtmncusBMW = 0,
                    mtmmedfMW = 0,    mncusMW  = 0,   mtmncusMW = 0,
                    mtmmedfBM = 0,    mncusBM = 0,    mtmncusBM = 0))
  ...
  orders = left_join(left_join(left_join(left_join(left_join(orders, 
    ...
    replace_na(list(mtmmedfBC1 = 0,    mncusBC1 = 0,      mtmncusBC1 = 0,   mtmmedfBC2 = 0,
                    mncusBC2 = 0,      mtmncusBC2 = 0,    mtmmedfBC3 = 0,   mncusBC3 = 0,
                    mtmncusBC3 = 0,    mtmmedfBC123 = 0,  mncusBC123 = 0,   mtmncusBC123 = 0,
                    mtmmedfBMC123 = 0, mncusBMC123 = 0,   mtmncusBMC123 = 0  ))
  ...
  orders = left_join(left_join(left_join(left_join(orders, 
    ...
    replace_na(list(mtmmedfMC1 = 0,  mncusMC1 = 0,   mtmncusMC1 = 0, 
                    mtmmedfMC2 = 0,  mncusMC2 = 0,   mtmncusMC2 = 0,   mtmmedfMC3 = 0,
                    mncusMC3 = 0,    mtmncusMC3 = 0, mtmmedfMC123 = 0, mncusMC123 = 0,
                    mtmncusMC123 = 0 ))
  ...
  newprice_by_item = orders %>% group_by(itemID) %>%
    summarise(newpriceI = mean(new_price)) %>% as.data.frame()
  newprice_by_week_item = orders %>% group_by(itemID, week) %>%
    summarise(newpriceIW = mean(new_price)) %>% as.data.frame()
  ...
  for (i in c(1:length(nname))){
    orders = left_join(left_join(left_join(left_join(left_join(left_join(orders, 
                                 ...
                                 newprice_by_item %>% 'colnames<-'(c(nname[i], paste(nname[i],"_newpriceI", sep = ""))), by = c(nname[i])),
                       newprice_by_week_item %>% 'colnames<-'(c(nname[i], "week", paste(nname[i],"_newpriceIW", sep = ""))), by = c(nname[i], "week"))
    ...
    orders[is.na(orders[,paste(nname[i],"_mncusI", sep = "")]), paste(nname[i],"_mncusI", sep = "")] = 0
    orders[is.na(orders[,paste(nname[i],"_morderI", sep = "")]), paste(nname[i],"_morderI", sep = "")] = 0
    orders[is.na(orders[,paste(nname[i],"_newpriceI", sep = "")]), paste(nname[i],"_newpriceI", sep = "")] = 0
    orders[is.na(orders[,paste(nname[i],"_newpriceIW", sep = "")]), paste(nname[i],"_newpriceIW", sep = "")] = 0
  ...
  if(logtr){
    orders12 = orders %>% mutate(logorder = log(order+1),
                                 lag1_order = log(lag1_order+1),
                                 ...
                                 npromoIW_npromoIW = log(npromoIW^2 + 1)) %>% select(final_var)  
  }else{
    orders12 = orders %>% mutate(logorder = log(order+1),
                                 npromoIW_npromoIW = npromoIW^2) %>% select(final_var)
  }

  # imp_var = names(orders12)[!(names(orders12) %in% c(nname, "brand", "tr_ts", "itemID", "week", "order", 
  ...
  if(logtr){
    write.csv(x = tr_dt, file = paste(path, "final_tr_std_log.csv", sep = ""), row.names = F)
    write.csv(x = ts_dt , file = paste(path, "final_ts_std_log.csv", sep = ""), row.names = F)
    write.csv(x = orders12 , file = paste(path, "final_tr_ts_log.csv", sep = ""), row.names = F)
    write.csv(x = orders12_std, file = paste(path, "final_tr_ts_std_log.csv", sep = ""), row.names = F)
  }else{
    write.csv(x = tr_dt, file = paste(path, "final_tr_std.csv", sep = ""), row.names = F)
    write.csv(x = ts_dt , file = paste(path, "final_ts_std.csv", sep = ""), row.names = F)
    write.csv(x = orders12 , file = paste(path, "final_tr_ts.csv", sep = ""), row.names = F)
    write.csv(x = orders12_std, file = paste(path, "final_tr_ts_std.csv", sep = ""), row.names = F)
  }

  
  # orders = orders %>% select(-c(nname))  
  rm(promo_by_item, tmncus_by_itm, mean_op_by_itm, promo_by_week_item, newprice_by_item, newprice_by_week_item)
  rm(items, ncust, comp_supp_features, promo, orders2w, week, orders, mean_tr_dt, sd_tr_dt)
  
  # aa = data.frame(aa = colSums(is.na(orders)))
  ###########################################
  
  # aa = cor(orders[orders$tr_ts==1,]) %>% as.data.frame() %>% select("order") %>% as.data.frame()
  # bb = rownames(aa)[which((abs(aa$order)>=.1) & (abs(aa$order)<1))]
  # orders1 = orders %>% select(c(names(orders)[1:13], bb))
  # 
  # orders1 = orders1 %>% arrange(itemID, week) %>% group_by(itemID) %>%
  #   mutate(lag1_order = lag(order, n = 1L),
  #          lag2_order = lag(order, n = 2L),
  #          lag3_order = lag(order, n = 3L),
  #          lag4_order = lag(order, n = 4L),
  ...
  # 
  # comp_supp_features = read.csv(paste(path, "comp_supp_features.csv", sep = ""), header = TRUE, sep = ",")[,-1]
  # nname1 = c("comp1",                 "comp2",                  "comp3",
  #            "comp4",                 "comp5",                  "comp6",
  #            "comp7",                 "comp8",                  "comp9",
  #            "comp10")
  # orders1 = left_join(orders1, comp_supp_features %>% 
  #                       select(c(nname1, "itemID")), by = "itemID") %>% 
  #   replace_na(list(comp3 = 0,    comp2 = 0,    comp1 = 0))
  # 
  # 
  # 
  # for (i in c(1:length(nname1))){
  #   orders1[,paste(nname1[i],"_order", sep = "")]= comp_supp_func(dt = orders1,
  #                                                                 var_name = nname1[i])
  # }
  ...
  # ts_dt[,imp_var] = scale(ts_dt[,imp_var], mean_tr_dt, sd_tr_dt)
  # rm(comp_supp_features, mean_tr_dt, sd_tr_dt)
  
  # write.csv(x = orders1 %>% mutate(logorder = log(order+1)), file = paste(path, "orders_subset.csv", sep = ""), row.names = F)
  # write.csv(x = tr_dt, file = paste(path, "train_dt_std.csv", sep = ""), row.names = F)
  # write.csv(x = ts_dt, file = paste(path, "test_dt_std.csv", sep = ""), row.names = F)
  # 
...
imp_var = c(   "customerRating",           "new_price",             "recommendedRetailPrice",
               "lag1_order",               "lag2_order",
               "comp1_order",              "comp2_order",           "comp1_npromoIW",
               "comp10_npromoIW",          "comp2_npromoIW",        "comp3_npromoIW",
               "comp3_order",              "comp4_npromoIW",        "comp5_npromoIW",
               "comp6_npromoIW",           "comp7_npromoIW",        "comp8_npromoIW",
               "comp9_npromoIW",           "supp1_npromoIW",        "supp2_npromoIW",
               "mncusI",                   "mncusIW",               "morderI",
               "mtmncusBMC123",            "npromoBMC123W",         "npromoBMW",
               "npromoI",                  "npromoIW",              "npromoIW_npromoIW")

ggplot(dt, aes(x = lag1_order, fill = as.factor(tr_ts))) + geom_density(alpha = 0.5)
ggplot(dt, aes(x = logorder, fill = as.factor(tr_ts))) + geom_density(alpha = 0.5)
ggplot(dt, aes(x = comp1_npromoIW, fill = as.factor(tr_ts))) + geom_density(alpha = 0.5)
...
train_dt = read.csv(file = paste(path, "train_dt_1.csv", sep = ""), header = T)
test_dt = read.csv(file = paste(path, "test_dt.csv", sep = ""), header = T)
...
aatr = as.data.frame(colSums(is.na(train_dt)))
aats = as.data.frame(colSums(is.na(test_dt)))
